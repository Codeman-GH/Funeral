/*$(function() {
	...
	// Add logic for search filter two
	var searchVal, comparator, hasMatch, regex, showAll;
	$('#search-worker2').on('keyup', function() {
		searchVal = $(this).val();
		...
		$.each($('.worker-nodule'), function() {
			comparator = $(this).find('.first_name, .last_name, .Main_job_description, .date_joined, .City_or_Town, .likes').text();
